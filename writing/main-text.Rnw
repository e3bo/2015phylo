...
\title{A regularization approach to efficiently fitting regression
  models of the transition rates of discrete traits with an
  application to viral phylogeography}
  
\author{Eamon O'Dea \and Shweta Bansal}
...
<<load-results,include=FALSE,eval=FALSE>>=
