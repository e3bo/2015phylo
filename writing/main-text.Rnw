...
detected in neighboring Canada and
Mexico~\cite{hill2014,efsa2014}. The circulating strains cause severe
diarrhea and vomiting~\cite{jung_pathology_2014}, and more than 90
percent of unweaned pigs may die during an outbreak on a
farm~\cite{stevenson2013}. Those pigs that do survive infection have
been found to be less productive~\cite{alvarez2015}.
...
weak points exist is clear from the occurrence of PEDV outbreaks
without any clear cause on farms with rigorous biosecurity
programs~\cite{stevenson2013}. Plausible sources of infection include
aerosolized particles, contaminated fomites such as feed and transport
trailers, animals with subclinical infections, and milk from affected
sows~\cite{jung2015}. Genomic analyses
...
Refs.~\cite{nelson2011,lemey2014,magee2014}.
...
North America~\cite{vlasova2014}. Thus we generated separate alignments
for S INDEL and non-S INDEL sequences and estimate separate trees for
them.  We used mafft to generate the alignments~\cite{katoh2013}. The
alignments were run through Gblocks~\cite{castresana2000} to identify
...
and indels. The alignments were also checked visually for evidence of
...
TODO describe rationale for constant population size gamma prior

with one notable exception. We chose the doubling time
...
%  \includegraphics[width=\textwidth]{fig1}
  \caption{{\bf The resolution of clades and clade ages for the PEDV
    ...
    time scale. The S INDEL sequences form the monophyletic group at
    the bottom that diverges from the rest of the sequences at the
    root of the tree. These trees were based on
    \Sexpr{#uniquePatterns[sIndel]} unique columns in the alignment of
    S INDEL sequences and \Sexpr{#uniquePatterns[nonsIndel]} in the
    alignment of the other sequences.
  }\label{densitree}
\end{figure}
