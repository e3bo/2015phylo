...
\title{Identification of variables associated with geographic
  movements of PEDV with a novel regularized regression approach}
...
We used BEAST~\cite{drummond2012} to obtain a sample of evolutionary
trees from the sequences in the alignment. Because PEDV in North
America is believed to have spread rapidly following its
introduction~\cite{lowe2014}, we evaluated the likelihood of
coalescent trees according to the exponential growth
model~\cite{griffiths1994,drummond2002} of effective population
size. We used a log-normal, relaxed molecular clock model
\cite{drummond2006} to allow for variation in substitution rates among
branches.
...
# load("../work/results.RData")

answer <- 7

@ 

We find \Sexpr{answer} important variables.
