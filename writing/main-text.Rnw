...
for the times of ancestral nodes of the tree. We also assume that
substitutions occur according to a molecular clock with a constant
rate and that the relative rates of substitutions between different
bases occur according to the HKY85 model~\cite{hasegawa1985}. We allow
for random variation in rates among sites using the standard
approximation to the gamma distribution~\cite{yang1994}, with four
discrete rate categories in our approximation.
...
substitution model parameters, as previous estimates from severe
acute respiratory syndrome (SARS) coronavirus~\cite{salemi2004} were
in relatively high density locations of the priors. We placed a normal
...
swine. This prior was truncated at zero since rates must be positive.
For a prior on the coalescent population size, we used a gamma
distribution with a shape of 3 and a scale of 0.3. This prior was
chosen based on our belief that it is unlikely that the most recent
common ancestor of either the S INDEL and non-S INDEL sequences
existed much earlier than the date of the first known
U.S. outbreak. For two sequences not having month and days of
collection, we placed a uniform prior on their day of collection over
a window of 86 days considered likely to include the true collection
date. Our BEAST XML input files have been archived on the web (TODO
make sure full build is working, make archive, add citation).
...
\begin{figure}[h]
  ...
  \caption{{\bf The resolution of clades and clade ages for the PEDV
    ...
    were based on \Sexpr{uniquePatterns['sIndel']} unique columns in
    the alignment of S INDEL sequences and
    \Sexpr{uniquePatterns['nonsIndel']} in the alignment of the other
    sequences.  }\label{densitree}
\end{figure}

\begin{figure}[h]
\includegraphics{fig2}
\caption{{\bf The S INDEL and non-S INDEL strains likely diverged
    before the first reported U.S. outbreak.}  }\label{rootheights}
\end{figure}
...
maximum likelihood estimates for regression coefficients rather than
posterior distributions. The maximum likelihood estimates are not as
informative as the posterior distributions, but they may be sufficient
for many analyses and have the advantage of being faster to
