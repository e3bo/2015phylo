...
sequences a posterior sample of evolutionary trees with branch lengths
...
distributions seemed fairly smooth, traces appeared to have constant
means and variances. We increased the length of the chains until all
effective sample sizes exceeded 200, and ended up with a chain 100
million states in which the minimum effective sample size was 408.

\section*{Results}

<<load-results,include=FALSE>>=

load("../work-sshd/mle.RData")
