...
Refs.~\cite{nelson2011,lemey2014,magee2014} . A regression model is
...
that their most recent common ancestor predates their introduction to
North America~\cite{vlasova2014}. For our anlaysis we are primarily
interested in the evolutionary history of the strains within North
America ans thus we generated separate alignments for S INDEL and
non-S INDEL sequences and estimate separate trees for them.  We used
mafft to generate the alignments~\cite{katoh2013}. The alignments were
run through Gblocks~\cite{castresana2000} to identify any very poorly
conserved, potentially misleading segments. Gblocks removed only
regions with many gaps such as the ends of the alignment and
indels. The alignments were also checked visually for evidence of
alignment errors using CLUSTAL~X~\cite{thompson1997}.
...
existed much earlier than the date of the first known U.S. outbreak
because in the past succesful introductions of PEDV have hardly been
frequent. For two sequences not having month and days of collection,
we placed a uniform prior on their day of collection over a window of
86 days considered likely to include the true collection date. Our
BEAST XML input files have been archived on the web (TODO make sure
...
sequence location data. It is a maximum simulated likelihood (MSL) variant of the
fully Bayesian approach introduced in Ref.~\cite{lemey2014}.
TODO find good reference for MSL

We consider that the true trees are unosbserved, multivariate random
variables and suppose that the posterior samples of tree accurately
represent simulations of these variables. Recall that each tree
...
Markov chain are collected in a $K \times K$ matrix
$\bm{\Lambda}$, where $\Lambda_{i,j}$ gives the rate of change
from state $i$ to state $j$. We assume these infinitesimals are
...
$\Lambda$, the likelihood of the observed data for a given tree may be
efficiently calculated with the same dynamic programming algorithm
used to calculate the likelihood of a column in a sequence alignment
for a given substitution model. We then take the average of these
likelihoods over a set of simulated trees to approximately integrate
out uncertainty in the tree, arriving at the simulated likelihood.

TODO describe rationale for number of simulations
...
We found the maximum simulated likelihood regression coefficients
...
branch, as determined by the best-fit model. The output was a set of
locations for the tips of the tree. We employed the
\texttt{simulate.msa} function of rPhast to carry out \Sexpr{bs$R} of
these simulations. For each simulation replicate, we obtained MSL
estimates of the regression coefficients. Confidence intervals for
each coefficient were calculated as the interval spanned by the middle
95\% of these bootstrap estimates.
...
beliefs about the age of the root nodes than did our prior. The age of
the common ancestor of the S INDEL and non-S INDEL strains would be
farther back than these root nodes. Thus these results are consistent
with our assumption that this node likely occured outside of North
...
intervals from many simulations in a relatively short period of
time. Also, speed may be essential in shortening the often numerous
...
of shipment flows with migration rates is a necessary, causal
one. Unequal sampling efforts among states could be biasing our
estimates, for example. Another limitation to this analysis is the
simplicity of the model we used to estimate the tree, which may not be
taking full advantage of the information of the data. We opted for a
relatively simple model that would allow us more confidence that our
Markov chain sampler had converged to the posterior distribution of
trees, as diagnosing lack of convergence to the posterior distribution
of trees is still an active area of research~\cite{whidden2015}. In
sum, improvements in estimation of the tree, additional data
collection, the development of predictive variables relevant to
sampling and transmission mechanism could all improve the accuracy of
our estimates and the predictive performance of our model.
